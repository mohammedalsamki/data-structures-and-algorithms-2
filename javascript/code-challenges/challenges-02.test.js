...
const raisedToTheThird = (arr) => {
  ...
  const newArray = arr.map(element => (Math.pow(element, 3)));
  return (newArray)
};
...
const mapTwoToThe = (arr) => {
  ...
  const newArray = arr.map(element => (Math.pow(2, element)));
  return (newArray);
...
const extractStats = (arr) => {
  ...
  arr.map(element =>{
    ...
    newArray.push({name :element.stat.name,total: element.effort+element.baseStat});
  });
  return(newArray);
