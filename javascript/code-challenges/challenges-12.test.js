...
const validatePin = (pin) => {
  ...
  var number = "" + pin;
  let state;
  let pattern = number.match(/^[0-9]+$/);

  if (pattern && number.length === 4) {
    state = true;
  } else if (typeof pin === "string" && pin.length === 4 && pattern === null) {
    state = false;
  } else {
    state = false;
  }
...
const validateWord = (word) => {
  ...
  var strig = "" + word;
  let state;
  let pattern = strig.match(/^[a-zA-Z]{5,10}$/);

  if (pattern) {
    state = true;
  } else if (typeof (word === "number") && pattern === null) {
    state = false;
  } else {
    state = false;
  }
...
const hasNumber = (string) => {
  // Solution code here...
  let state;
  let pattern = /[a-zA-z]+[0-9]+/.test(string);
  console.log(pattern);
  if (pattern) {
    state = true;
  } else if (typeof (string === "number") && pattern === null) {
    state = false;
  } else {
    state = false;
  }
...
const validateEmail = (email) => {
  // Solution code here...
  let state;
  let pattern =  /^\w+\.?\w+@\w+.(com|net|org)$/.test(email)
  console.log(pattern)
  if (pattern) {
    state = true;
  } else if (typeof (email === 'number') && pattern === null) {
    state = false;
  } else {
    state = false;
  }
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let state;
  let pattern =   /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/.test(phoneNumber)
  console.log(pattern)
  if (pattern) {
    state = true;
  } else if (typeof (phoneNumber === 'number') && pattern === null) {
    state = false;
  } else {
    state = false;
  }
