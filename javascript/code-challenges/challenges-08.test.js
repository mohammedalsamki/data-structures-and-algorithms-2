...
const sortByChildren = (charArray) => {
  ...
  newArray.forEach((element, idx) => {
    NumOfChild.push(element.children.length);
    newArray[idx].sorter = NumOfChild[idx];
  });
...
const isCapitalized = (str) => {
  ...
  let final = newArrayList.join(" ");
  let finalfinal = final.replace(/[^a-zA-Z ]/g, "").split(" ");

  if (finalfinal.length === 1) {
    let result2 = [];
    return result2;
  } else {
    let result = finalfinal;
    return result;
  }
};
/* ------------------------------------------------------------------------------------------------
...
const citiesAtoJ = (arr) => {
  ...
  const C = arr.filter((element) => {
    return element.startsWith("C");
  });
  const D = arr.filter((element) => {
    return element.startsWith("D");
  });
  const E = arr.filter((element) => {
    return element.startsWith("E");
  });
  const F = arr.filter((element) => {
    return element.startsWith("F");
  });
  const G = arr.filter((element) => {
    return element.startsWith("G");
  });
  const H = arr.filter((element) => {
    return element.startsWith("H");
  });
  const I = arr.filter((element) => {
    return element.startsWith("I");
  });
  const J = arr.filter((element) => {
    return element.startsWith("J");
  });

  var M1 = A.concat(B, C);
  var M2 = M1.concat(D, E);
  var M3 = M2.concat(F, G);
  var M4 = M3.concat(H, I);
  var M5 = M4.concat(J);
